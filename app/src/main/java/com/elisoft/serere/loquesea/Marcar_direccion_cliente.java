...
import com.elisoft.serere.AndroidPermissions;
import com.elisoft.serere.Constants;
import com.elisoft.serere.GeocodeAddressIntentService;
import com.elisoft.serere.NewGPSTracker;
...
public class Marcar_direccion_cliente extends AppCompatActivity  implements OnMapReadyCallback, View.OnClickListener, GoogleApiClient.OnConnectionFailedListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_marcar_drieccion_cliente);
...
// localizacion automatica
        ...
        try{
            ...
            latitud_buscador= Double.parseDouble(carrito.getString("latitud_entrega","0"));
            longitud_buscador= Double.parseDouble(carrito.getString("longitud_entrega","0"));
            et_direccion.setText(carrito.getString("direccion_entrega",""));
...
//bicacion del button de Myubicacion de el fragento..
                ...
                switch (status.getStatusCode()) {
                    ...
                    case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                        try {
                            ...
                            status.startResolutionForResult(Marcar_direccion_cliente.this, PETICION_CONFIG_UBICACION);
    ...
    private void startLocationUpdates() {
        if (ActivityCompat.checkSelfPermission(Marcar_direccion_cliente.this,
            ...
            LocationServices.FusedLocationApi.requestLocationUpdates(
                    apiClient, locRequest, Marcar_direccion_cliente.this);
        }
    }
    ...
    private void updateUI(Location loc) {
        if (loc != null) {
            ...
            if(direccion.length()==4 && sw_acercar_a_mi_ubicacion!=3)
            {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.ll_agregar:
                String direccion_texto=et_direccion.getText().toString().trim();
                if(direccion_texto.length()==0){
                    et_direccion.setText(s_direccion.trim());
                }

                SharedPreferences carrito = getSharedPreferences(getString(R.string.sql_loquesea), MODE_PRIVATE);
                SharedPreferences.Editor ed_c = carrito.edit();
                ed_c.putString("latitud_entrega", String.valueOf(addressLatLng.latitude));
                ed_c.putString("longitud_entrega", String.valueOf(addressLatLng.longitude));
                ed_c.putString("direccion_entrega", et_direccion.getText().toString().trim()+tv_direccion.getText().toString().trim());
                ed_c.commit();
            ...
            case R.id.tv_direccion:
                et_direccion.setText(s_direccion);
                break;
                                    ...
                                    runOnUiThread(new Runnable() {
                                        public void run() {
                                            ...
                                            try {
                                                ...
                                                tv_direccion.setText(s_direccion.trim());
                                            }catch (Exception e)
                                            {
